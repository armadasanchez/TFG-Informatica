...
def run(gamma, beta, output, totalData, train_set, test_set,puzzle_elo_dict, user_objective = 'all', group_objective = 'all', puzzle_objective = 'all'):
    ...
    for i in idComplet.keys():
        ...
        competency_ELO.at[i, 'competency'] = round(learner_competency[key_split[1]][key_split[2]],3)
    ...
    dificultad = 0
    if puzzle_objective == 'Sandbox':
        dificultad = 0
    else:
        for key in puzzleDiffMean[puzzle_objective].keys():
            dificultad += normalized_question_difficulty[puzzle_objective][key]
        dificultad = round(dificultad, 3)

    resultadokcs = competency_ELO[(competency_ELO['user'] == user_objective) & (competency_ELO['group'] == group_objective)]
    elo = round(resultadokcs['competency'].sum(), 2)

    return elo, dificultad
